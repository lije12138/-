...
def seed_everything(seed):
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed); torch.cuda.manual_seed(seed); torch.backends.cudnn.deterministic = True
...
model = timm.create_model('densenet201', pretrained=False, num_classes=len(CONFIG['target_labels']))
state_dict = torch.load(os.path.join(WEIGHT_DIR, "densenet201/pytorch_model.bin"), map_location='cpu')
...
for epoch in range(30):
    ...
    for i, (imgs, _, labels) in enumerate(tqdm(train_loader, desc=f"DenseNet Epoch {epoch}")):
    ...
    pd.DataFrame(log_data).to_csv("training_log_densenet.csv", index=False)
    
    # --- 核心修改：基于 mAP 进行评估和保存 ---
    if mAP > best_map:
        best_map = mAP
        best_metrics = {"Model": "DenseNet201", "mAP": mAP, "MacroF1": f1, "AUC": auc}
